...
namespace CandidateProject.Controllers
{
    public class CartonController : Controller
    {
        ...
        public ActionResult DeleteConfirmed(int id)
        {
            ...
            var parent = db.Cartons.Include(p => p.CartonDetails)
             .SingleOrDefault(p => p.Id == id);

            foreach (var child in parent.CartonDetails.ToList())
              db.CartonDetails.Remove(child);

            db.Cartons.Remove(parent);
            db.SaveChanges();
            return RedirectToAction("Index");
        }
        ...
        public ActionResult AddEquipment(int? id)
        {
            ...
            var count = db.CartonDetails.Count(cd => cd.CartonId == id);

            if (count < 10)
            {
                var _equipment = db.Equipments
                .Where(c => !db.CartonDetails
            ...
            .Contains(c.Id))
             .Select(e => new EquipmentViewModel()
             {
                ...
                carton.Equipment = _equipment;                              
            }

            else 
            {
                ViewBag.Message = "Cannot add more equipments to this Carton";               
            }
